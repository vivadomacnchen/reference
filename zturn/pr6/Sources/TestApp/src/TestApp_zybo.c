...
// PS GPIO
#define INPUT_BANK XGPIOPS_BANK1   /* Bank 1 of the GPIO Device */
#define OUTPUT_BANK	XGPIOPS_BANK2  /* Bank 2 of the GPIO Device */
static int iPinNumber = 50; /*BTN4 is connected to MIO pin 50*/
// EMIO
int iPinNumberEMIO = 54; // EMIO Channel 0 maps to 54
u32 uPinDirectionEMIO = 0x1; // output
...
static XGpioPs_Config *PsGpioConfigPtr;
...
static void IntrHandler(void *CallBackRef, int Bank, u32 Status)
{
	...
	// Do nothing if the intr is generated for a different pin.
	if (XGpioPs_ReadPin(Gpio,iPinNumber) != 1) {
		return;
	}

	// Send a pulse.
   	XGpioPs_WritePin(Gpio,iPinNumberEMIO,0);
   	XGpioPs_WritePin(Gpio,iPinNumberEMIO,1);
}
				...
				u16 GpioIntrId)
{
	...
	// Connect the device driver handler that will be called when an interrupt for
	// the device occurs, the handler performs the specific interrupt processing for the device.
	...
	// Enable low level interrupt for MIO pin 50 in bank 1.
	XGpioPs_SetIntrTypePin(Gpio, iPinNumber, XGPIOPS_IRQ_TYPE_EDGE_RISING);

	// Set the handler for gpio interrupts.
	XGpioPs_SetCallbackHandler(Gpio, (void *)Gpio, IntrHandler);


	// Enable the GPIO interrupt of Pin 50 (Bank 1).
	XGpioPs_IntrEnablePin(Gpio, iPinNumber);
...
int main()
{
	...
	PsGpioConfigPtr = XGpioPs_LookupConfig(XPAR_XGPIOPS_0_DEVICE_ID);
	if (PsGpioConfigPtr == NULL) {
		return XST_FAILURE;
	}
	XGpioPs_CfgInitialize(&Gpio, PsGpioConfigPtr, PsGpioConfigPtr->BaseAddr);

	// Setup direction register of bank1 to be input as BTN4 is connected to Channel 50
	XGpioPs_SetDirection(&Gpio, INPUT_BANK, 0);

	//EMIO PIN Setting to Output port so we can pulse when BTN4 is pressed
	XGpioPs_SetDirectionPin(&Gpio, iPinNumberEMIO,uPinDirectionEMIO);
	XGpioPs_SetOutputEnablePin(&Gpio, iPinNumberEMIO, 1);
	XGpioPs_WritePin(&Gpio,iPinNumberEMIO,1);
